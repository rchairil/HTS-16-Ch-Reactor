# Support vector classifier (SVC) machine learning model and ternary diagram plotter for cesium-lead-bromide (CsxPbyBrz) system.  
...
#   4. The sheet "k_neighbors_X" will contain the observed Br-Cs-Pb mole ratios used for training the SVC. Copy-paste the observed and stoichiometric ratios from "ratios vs phases"
#   5. The sheet "k_neighbors_Y" will contain the observed Br-Cs-Pb phases used for training the SVC. Copy-paste the observed and stoichiometric phases from "ratios vs phases"
# This code's main outputs:
#   1. "ternary_grid.xlsx", an array representing the entire Cs-Pb-Br ternary mixing domain, and "support_vector_predictions.xlsx", the phase predictions based on the trained SVC. 
#      The fineness of the mesh can be easily adjusted.
#   2. A ternary plot containing observed, stoichiometric, and SVC-predicted phases represented as different colors. 
...
X_test.to_excel("X_test_svc.xlsx") # initialize X_test and/or delete old contents
y_train = [] 
...
y_pred.to_excel("train_result_svc.xlsx") # initialize Y_pred and/or delete old contents
...
rkf_indices.to_excel("rkf_indices_svc.xlsx", index = True)

# grid search for best SVC parameters (normalization penalty C, kernel type, polynomial degree)
def SVC_GridSearch():
    best_paras = pd.Series()
    best_scores = pd.Series()
    params = [
    {
    'C': [n for n in np.linspace(0.1,3,num=10)],
    'kernel': ['rbf', 'linear', 'poly'],
    'degree': [n for n in np.linspace(1,7,dtype=int)]
    }
    ]

    gridCV = GridSearchCV(estimator=SVC(), param_grid=params, scoring='accuracy', n_jobs=-1, cv=rkf)
    X_gs = X
    y_gs = y
...
# to optimize SVC hyperparameters - uncomment the two lines of code below; else, leave these commented out to reduce processing time
param_filename = "svc_GridSearch_results.xlsx"
pd.DataFrame(SVC_GridSearch()).to_excel(param_filename, index = True)

# main SVC training module
...
for n in range(rkf.get_n_splits(X, y)):
    ...
    # train the SVC model per split
    svc = SVC(C=1.066, kernel='poly', degree=5, random_state=42)
    svc.fit(X_train.values, np.array(y_train).ravel())
    models.append(svc)
    y_pred.append(svc.predict(X_test.values))

    # output the training and validation results
    fold_str = 'Fold no. ' + str(n+1)   
    y_pred = pd.DataFrame(y_pred).T
    with pd.ExcelWriter("train_result_svc.xlsx", mode='a', if_sheet_exists = 'replace') as ywriter:
        y_pred.to_excel(ywriter, sheet_name = fold_str)

    X_test = pd.DataFrame(X_test)
    with pd.ExcelWriter("X_test_svc.xlsx", mode='a', if_sheet_exists = 'replace') as xwriter: 
    ...
    if n == 0:
        ...
        accuracy_main.to_excel("accuracy_SVC.xlsx", sheet_name='accuracy')
...
X_lc = X
y_lc = y
display = LearningCurveDisplay.from_estimator(svc, X_lc.values, np.array(y_lc).ravel(), cv=25, shuffle=True, random_state=42, scoring='accuracy')
...
# apply the trained SVC model over the entire Cs-Pb-Br mixing space
y_u = svc.predict(X_u.values)
y_u = pd.DataFrame(y_u)
y_u.to_excel("support_vector_predictions.xlsx", index=False)
...
pio.write_image(fig, 'ternary_SVC.pdf',scale=1)