# K-neighbors classifier (KNC) machine learning model and ternary diagram plotter for cesium-lead-bromide (CsxPbyBrz) system.  
# This code maps the most likely phase from a Cs-Pb-Br reaction on the 16-ch reactor, based on its component mole ratio.
# Requirements:
#   1. An Excel sheet entitled "CsPbBr phase diagrams.xlsx" with sheets named "ratios vs phases", "k_neighbors_X", and "k_neighbors_Y"
...
#   4. The sheet "k_neighbors_X" will contain the observed Br-Cs-Pb mole ratios used for training the KNC. Copy-paste the observed and stoichiometric ratios from "ratios vs phases"
#   5. The sheet "k_neighbors_Y" will contain the observed Br-Cs-Pb phases used for training the KNC. Copy-paste the observed and stoichiometric phases from "ratios vs phases"
# This code's main outputs:
#   1. "ternary_grid.xlsx", an array representing the entire Cs-Pb-Br ternary mixing domain, and "k_neighbor_predictions.xlsx", the phase predictions based on the trained KNC. 
#      The fineness of the mesh can be easily adjusted.
#   2. A ternary plot containing observed, stoichiometric, and KNC-predicted phases represented as different colors. 
...
X_test.to_excel("X_test_knc.xlsx") # initialize X_test and/or delete old contents
y_train = [] 
...
y_pred.to_excel("train_result_knc.xlsx") # initialize Y_pred and/or delete old contents
...
rkf_indices.to_excel("rkf_indices_knc.xlsx", index = True)

# grid search for best KNC parameters (number of neighbors, distance weights, and leaf_size (if needed))
def knc_GridSearch():
    best_paras = pd.Series()
    best_scores = pd.Series()
    params = [
    {
    'n_neighbors': [n for n in np.linspace(3,30,num=20,dtype=int)],
    'weights': ['uniform'], # using distance weighting might result in overfitting (i.e. training learning curves with 100% accuracy over all samples in training set); use with caution
    'leaf_size': [n for n in np.linspace(2,400,dtype=int)]
    }
    ]

    gridCV = GridSearchCV(estimator=KNeighborsClassifier(), param_grid=params, scoring='accuracy', n_jobs=-1, cv=rkf)
    X_gs = X
    y_gs = y
...
# to optimize KNC hyperparameters - uncomment the two lines of code below; else, leave these commented out to reduce processing time
param_filename = "knc_GridSearch_results.xlsx"
pd.DataFrame(knc_GridSearch()).to_excel(param_filename, index = True)

# main KNC training module
...
for n in range(rkf.get_n_splits(X, y)):
    ...
    # train the k-neighbors classifier model per split
    knc = KNeighborsClassifier(n_neighbors=13, weights='uniform', leaf_size=2, n_jobs=-1)
    knc.fit(X_train.values, np.array(y_train).ravel())
    models.append(knc)
    y_pred.append(knc.predict(X_test.values))

    # output the training and validation results
    fold_str = 'Fold no. ' + str(n+1)   
    y_pred = pd.DataFrame(y_pred).T
    with pd.ExcelWriter("train_result_knc.xlsx", mode='a', if_sheet_exists = 'replace') as ywriter:
        y_pred.to_excel(ywriter, sheet_name = fold_str)

    X_test = pd.DataFrame(X_test)
    with pd.ExcelWriter("X_test_knc.xlsx", mode='a', if_sheet_exists = 'replace') as xwriter: 
    ...
    if n == 0:
        ...
        accuracy_main.to_excel("accuracy_KNC.xlsx", sheet_name='accuracy')
...
X_lc = X
y_lc = y
display = LearningCurveDisplay.from_estimator(knc, X_lc.values, np.array(y_lc).ravel(), cv=25, shuffle=True, random_state=42, scoring='accuracy')
...
# apply the trained KNC model over the entire Cs-Pb-Br mixing space
y_u = knc.predict(X_u.values)
y_u = pd.DataFrame(y_u)
y_u.to_excel("k_neighbor_predictions.xlsx", index=False)
...
# then copy the values from the "k_neighbor_predictions.xlsx" output and place them in the phase column of "CsPbBr phase diagrams.xlsx", sheet_name='ratios vs phases'
# axes customization
...
pio.write_image(fig, 'ternary_KNC.pdf',scale=1)